...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { Invoke-WebRequest -Uri '%SCRIPT_URL%' -UseBasicParsing -OutFile '%SCRIPT_PATH%'; & '%SCRIPT_PATH%' -ServerUrl '%API_URL%' -Ruangan '%RUANGAN%' -TahunInventaris '%TAHUN_INVENTARIS%' -NamaUser '%NAMA_USER%' -Kondisi '%KONDISI%'; exit $LASTEXITCODE } catch { Write-Host 'Gagal menjalankan pendataan:' $_.Exception.Message -ForegroundColor Red; exit 1 }"
