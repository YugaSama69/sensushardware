...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '%SCRIPT_URL%' -UseBasicParsing -OutFile '%SCRIPT_PATH%'; & '%SCRIPT_PATH%' -ServerUrl '%API_URL%' -Ruangan '%RUANGAN%' -TahunInventaris '%TAHUN_INVENTARIS%' -NamaUser '%NAMA_USER%'; exit \$LASTEXITCODE } catch { Write-Host 'Gagal menjalankan pendataan:' \$_.Exception.Message -ForegroundColor Red; exit 1 }"
