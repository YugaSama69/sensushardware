...
if (is_post()) {
    ...
    if ($action === 'update') {
        ...
        if (!$errors) {
            $historyRow = fetch_one($pdo, 'SELECT * FROM histori_barang WHERE id = :id AND tipe_transaksi = "keluar" LIMIT 1', ['id' => $historyId]);
            if (!$historyRow) {
                $errors[] = 'History barang keluar tidak ditemukan.';
            } else {
        ...
        if (!$errors && $historyRow && $item) {
            ...
            $stokBaru = $stokSekarang + (int) $historyRow['qty'] - $qtyKeluar;
            ...
            if ($stokBaru < 0) {
                ...
                try {
                    ...
                    $updateHistory->execute([
                        'qty' => $qtyKeluar,
                        'stok_sesudah' => (int) $historyRow['stok_sebelum'] - $qtyKeluar,
                        'nama_pengguna' => $namaPengguna,
                        'tujuan' => $tujuan,
                        'keterangan' => $keterangan,
                        'id' => $historyId,
                    ...
                    $pdo->commit();
                    set_flash('success', 'Riwayat barang keluar berhasil diperbarui.');
                    redirect('modules/transaksi/keluar.php');
                } catch (Throwable $throwable) {
                    if ($pdo->inTransaction()) {
                        $pdo->rollBack();
                    }
                    $errors[] = 'Gagal memperbarui riwayat barang keluar.';
                }
            }
        }
    }
    ...
    if ($action === 'delete') {
        ...
        if (!$errors && $historyRow && $item) {
            ...
            try {
                ...
                $pdo->commit();
                set_flash('success', 'Riwayat barang keluar berhasil dihapus.');
                redirect('modules/transaksi/keluar.php');
            } catch (Throwable $throwable) {
                if ($pdo->inTransaction()) {
                    $pdo->rollBack();
                }
                $errors[] = 'Gagal menghapus riwayat barang keluar.';
            }
        }
    }
}
                ...
                <form method="post" class="room-filter-form">
                    ...
                    <div class="mb-3">
                        ...
                        <input
                            ...
                            required
                        >
                        <datalist id="barangKeluarLookup" data-item-datalist>
                        ...
                        <div class="form-text">Gunakan filter barang agar daftar saran lebih detail sebelum memilih barang keluar.</div>
                    </div>
                    <div class="mb-3">
                    ...
                    <table class="table align-middle">
                        ...
                        <tbody>
                            <?php if (!$recentEntries): ?>
                                <tr>
                                    <td colspan="9" class="text-center text-muted py-4">Belum ada transaksi barang keluar.</td>
                                </tr>
                            <?php endif; ?>
                            <?php foreach ($recentEntries as $entry): ?>
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editKeluarModal<?= $entry['id']; ?>">Edit</button>
                                        <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deleteKeluarModal<?= $entry['id']; ?>">Hapus</button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
...
<?php foreach ($recentEntries as $entry): ?>
    <div class="modal fade" id="editKeluarModal<?= $entry['id']; ?>" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="post">
                    <div class="modal-header">
                        <h5 class="modal-title">Edit Riwayat Barang Keluar</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Tujuan Penggunaan</label>
                            <input type="text" name="tujuan" class="form-control" value="<?= e($entry['tujuan']); ?>" required>
                        </div>
                        <div class="mb-0">
                            <label class="form-label">Keterangan</label>
    ...
    <div class="modal fade" id="deleteKeluarModal<?= $entry['id']; ?>" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="post">
                    <div class="modal-header">
                        <h5 class="modal-title">Hapus Riwayat Barang Keluar</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <?= csrf_field(); ?>
                        ...
                        <input type="hidden" name="history_id" value="<?= $entry['id']; ?>">
                        <p>Yakin ingin menghapus transaksi barang keluar ini?</p>
                        ...
                        <div class="alert alert-warning mt-3 mb-0">
                            Menghapus transaksi ini akan menambah kembali stok barang saat ini.
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light" data-bs-dismiss="modal">Batal</button>
